...
\boxed{Tactic \Verb+rcases+}{Given a hypothesis or goal that is a disjunction \Verb+A∨B+ the tactic \Verb+rcases+ splits it into two sub-hypotheses or subgoals \Verb+A+ and \Verb+B+.\smallskip\\
...
\boxed{Tactic \Verb+rcases+}{\Verb+rcases h with ⟨v, h'⟩+ means given a hypothesis \Verb+h+, let \Verb+v+ be a value such that the new goal is \Verb+h'+.}\label{p.rcases}
...
\boxed{Tactic \Verb+norm\_num+}{Proves numerical equalities and inequalities that do not use variables.}
