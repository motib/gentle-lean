...
\boxed{Tactic \Verb+let+}{\Verb+let+ introduces a definition whose scope is local.}
...
\boxed{Tip right-to-left}{The symbol \Verb+<|+ means to compute the expression to its right and pass it to the left.}
...
\end{Verbatim}
\boxed{Tip negation}{A negation \Verb+¬p+ is \emph{defined} as \Verb+p → False+. The tactic \Verb+intro+ makes \Verb+p+ into a hypothesis and \Verb+False+ into the goal. When there are no more goals, \Verb+¬p+ has been proved (by contradiction).}
...
\boxed{Tactic \Verb+use+}{Given an existential goal \Verb+∃ c A(c)+, this tactic introduces a free variable for the bound variable \Verb+c+ to form a new goal. It will also attempt to prove the goal using the hypotheses.}
...
\boxed{Tactic \Verb+sorry+}{Proves any theorem.}
