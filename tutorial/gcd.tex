...
\boxed{Tactic \Verb+apply+}{If you have a theorem \Verb+P→Q+ and the goal matches\footnote{Technically, if the goal \emph{unifies} with \Verb+Q+, but we don't explain the concept here.} \Verb+Q+ then \Verb+apply ⊢P→Q+ removes the goal \Verb+Q+ and adds \Verb+P+ as the new goal.\smallskip\\
...
\boxed{Tip \Verb+done+}{All proofs should be terminated by \Verb+done+. This is not necessary but if your proof is not complete, \Verb+done+ will display a message.}

\boxed{Tip Division}{The division operator in Lean is not the \Verb+|+ symbol on your keyboard, but a similar Unicode symbol \Verb+∣+ obtained by typing \Verb+\symbol{92}|+.}
